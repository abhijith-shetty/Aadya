      ...
      <div class="row">
        <div class="col-md-2 block">
          ...
          <table class="table">
            <?php foreach ($apiList as $item) { ?>
              <tr>
                <td><a
                    href="<?php echo getComponentUrl('admin', 'console', array("methodName" => $item)); ?>"><?php echo $item; ?></a>
                </td>
              </tr>
            <?php } ?>
          </table>
        ...
        <div class="col-md-5 block">
          <?php if (!empty($_GET['methodName'])) { ?>
            <h4>Input Parameters: <small><a
                  href="<?php echo getComponentUrl('admin', 'manage', array("methodName" => $item)); ?>"
                  class="pull-right">Manage this API</a></small></h4>
            <form id="rest-form" name="restform">
              <table class="table">
                <tr>
                  <td>methodName:</td>
                  <td><strong><?php echo $_GET['methodName']; ?></strong></td>
                </tr>
                <tr>
                  <td>requestMethod:</td>
                  <td><strong><select id="requested-type">
                        <option value="GET">GET</option>
                        <option value="POST">POST</option>
                      </select></strong></td>
                </tr>
                <?php foreach ($params as $param) { ?>
                  <?php if (isset($param['showInConsole']) && $param['showInConsole'] == false) {
                    continue;
                  } ?>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td>
                      <input type="<?php echo $param['type'] ?? ''; ?>" name="<?php echo $param['name']; ?>"
                             id="<?php echo $param['name']; ?>"
                             value="<?php echo isset($param['default']) ? $param['default'] : ""; ?>"
                             class="col-md-12 text-box"/>
